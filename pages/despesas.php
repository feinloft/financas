    ...
    if (isset($_POST['save_movimentacao'])) {
        ...
        if (empty($descricao) || empty($categoria_id) || empty($valor)) {
            ...
            if ($id) {
                ...
                $stmt = $pdo->prepare("INSERT INTO movimentacoes (usuario_id, categoria_id, tipo, data, valor, descricao, observacoes) VALUES (?, ?, 'despesa', ?, ?, ?, ?)");
...
$stmt = $pdo->query("SELECT * FROM categorias WHERE tipo IN ('despesa', 'ambos') ORDER BY nome ASC");
...
$query = "SELECT m.*, c.nome as categoria_nome, c.cor as categoria_cor 
          ...
          WHERE m.usuario_id = ? AND m.tipo = 'despesa' 
