...
<div class="header-actions" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
    <h3>Receitas</h3>
    <a href="index.php?page=receitas&action=new" class="btn btn-success">Nova Receita</a>
</div>

<?php if (isset($_GET['action']) && ($_GET['action'] === 'new' || $_GET['action'] === 'edit')): ?>
<div class="card">
    <h4><?= $_GET['action'] === 'edit' ? 'Editar' : 'Nova' ?> Receita</h4>
    <form action="index.php?page=receitas&mes=<?= $mes_filtro ?>&ano=<?= $ano_filtro ?>" method="POST" class="form-inline" style="margin-top: 1rem;">
        ...
        <div class="form-group" style="flex: 2; min-width: 200px;">
            ...
            <input type="text" name="descricao" class="form-control" value="<?= s($edit_data['descricao'] ?? '') ?>" placeholder="Ex: Salário" required>
        </div>
        ...
        <div class="form-group">
            ...
            <a href="index.php?page=receitas&mes=<?= $mes_filtro ?>&ano=<?= $ano_filtro ?>" class="btn" style="background: #eee;">Cancelar</a>
        </div>
    </form>
</div>
<?php endif; ?>
...
<div class="card">
    ...
    <div class="table-responsive" style="margin-top: 2rem;">
        <table>
            ...
            <tbody>
                <?php $total_page = 0; ?>
                <?php foreach ($receitas as $r): $total_page += $r['valor']; ?>
                    <tr>
                        ...
                        <td style="color: var(--success); font-weight: 600;"><?= formatarMoeda($r['valor']) ?></td>
                        <td>
                            <a href="index.php?page=receitas&action=edit&id=<?= $r['id'] ?>&mes=<?= $mes_filtro ?>&ano=<?= $ano_filtro ?>">
                                <i class="fas fa-edit" style="color: var(--accent);"></i>
                            </a>
                            &nbsp;
                            <form action="index.php?page=receitas&mes=<?= $mes_filtro ?>&ano=<?= $ano_filtro ?>" method="POST" style="display:inline;" onsubmit="return confirm('Excluir esta receita?');">
                                <input type="hidden" name="csrf_token" value="<?= gerarCSRF() ?>">
                ...
                <?php if (empty($receitas)): ?>
                    <tr><td colspan="5" style="text-align:center;">Nenhuma receita encontrada para este período.</td></tr>
                <?php else: ?>
                    <tr style="background: #f8f9fa; font-weight: bold;">
                        <td colspan="3" style="text-align: right;">Total do Período:</td>
                        <td colspan="2" style="color: var(--success);"><?= formatarMoeda($total_page) ?></td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
